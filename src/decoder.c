...
/*hepeisong*/
//static int MODE_NOT_CODED_GMC_count = 0;
//static int MODE_NOT_CODED_count = 0;
static int pframeNum = 0;
...
/*    if (iframeFlag == 1 && i < 4)
    {
      FILE *fp;
      char filename[50] = "";
	sprintf(filename, "decode%d.txt", frameNum);
//      sprintf(filename, "%d.txt", frameNum);
      if (fp = fopen(filename, "a+"))
      {
        int count;
        for (count = 0; count < 64; count++)
          fprintf(fp, "%d ", block[i * 64 + count]);
        fprintf(fp, "\n");
        fclose(fp);
      }
    }
*/
    ...
    /*hepeisong,20150306*/
    if (i < 4 && iframeFlag == 0)  //prevent from outputing intra blocks of IVOP
    {
      FILE *fp;
      char filename[50] = "";

	  sprintf(filename, "Pframe_ver%3d.txt", pframeNum);
	  if (fp = fopen(filename, "a+"))
	  {
			fprintf(fp,"%d %d %d %d %d ",x_pos,y_pos,pMB->mode,0,0); // MVs are set to zero.

			int count;
//			for (count = 0; count < 64; count++) /*20150420*/
			fprintf(fp,"%d ",0); //set the value of residual to zero, 20150420
			fprintf(fp,"\n");
			fclose(fp);
	  }
    }
...
//	  sprintf(filename, "Pframe_residual%3d.txt", pframeNum);
//	  if (fp = fopen(filename, "a+")){
			...
			int count;
			for (count = 0; count < 64; count++)
				intra_sum = intra_sum + data[i * 64 + count];
			fprintf(fp, "%d\n", intra_sum);
			fclose(fp);*/

//			fprintf(fp,"%d %d ",x_pos,y_pos);
//			fprintf(fp,"%d ",pMB->mode);
//			fprintf(fp,"%d %d ",pMB->mvs[0].x,pMB->mvs[0].y);
//			int count;
//			for (count = 0; count < 64; count++)
//				intra_sum = intra_sum + abs(data[i + count]);
//			fprintf(fp,"%d\n",intra_sum);
//			fclose(fp);
        ...
        const uint32_t y_pos)
{
  ...
  typedef void (*get_inter_block_function_t)(
      ...
      const MACROBLOCK * pMB, /*add by hepeisong*/
      const uint32_t x_pos, /*add by hepeisong*/
      const uint32_t y_pos, /*add by hepeisong*/
	  int i,
	  int pframeNum); /*add by hepeisong*/
  ...
  for (i = 0; i < 6; i++) {
    ...
    if (cbp & (1 << (5 - i))) {
      ...
      start_timer();
      get_inter_block(bs, &data[0], direction, iQuant, get_inter_matrix(dec->mpeg_quant_matrices), pMB, x_pos, y_pos, i, pframeNum);
      stop_coding_timer();
      ...
      /*hepeisong,20150109: output the motion residual data[0] when i = 0~3(luminance)*/
      /* --- macroblock modes --- */
/*
      #define MODE_INTER	0
...
/******************************** comment in 20150305  hepeisong***********************************************/
      if (i < 4){
        /*motion residual*/
        	int residual_abs_sum = 0;
			FILE *fp;
			char filename[50] = "";
...
*/
			...
			{ /*
			  ...
			  int count;
		      for (count = 0; count < 64; count++)
		      	  residual_abs_sum = residual_abs_sum + abs(data[0 + count]);
			  fprintf(fp,"abs_motion_residual_sum:%d\n",residual_abs_sum);
			  fclose(fp);*/


				fprintf(fp,"%d %d ",x_pos,y_pos);
				fprintf(fp,"%d ",pMB->mode);
				fprintf(fp,"%d %d ",pMB->mvs[i].x,pMB->mvs[i].y);
				int count;
				for (count = 0; count < 64; count++)
					residual_abs_sum = residual_abs_sum + abs(data[0 + count]);
				fprintf(fp,"%d\n",residual_abs_sum);
			  ...
			  //for (count = 0; count < 64; count++)
				  //residual_abs_sum = residual_abs_sum + abs(data[0 + count]);
			  //fprintf(fp,"%d %d %d %d %d %d\n", x_pos, y_pos， pMB->mode， pMB->mvs[0].x, pMB->mvs[0].y， residual_abs_sum);
			  //fprintf(fp,"%d %d %d\n", x_pos, y_pos， pMB->mode);
			  //fclose(fp);
			}
		...
		const int bvop)
{
  ...
  if (cbp == 0){ //including MBs: P-MB with zero cbp and MODE_NOT_CODED MBs
 // if (cbp == 0 && pMB->mode != MODE_NOT_CODED){
 /******************************** comment in 20150420  hepeisong***********************************************/
	  for (i = 0; i < 4; i++){
  ...
  if (cbp)
    //decoder_mb_decode(dec, cbp, bs, pY_Cur, pU_Cur, pV_Cur, pMB);
	  decoder_mb_decode(dec, cbp, bs, pY_Cur, pU_Cur, pV_Cur, pMB, x_pos, y_pos);
}
...
/*hepeisong*/
/*  int MODE_NOT_CODED_GMC_count = 0;
  int MODE_NOT_CODED_count = 0;*/
  pframeNum++;
  iframeFlag = 0;
 ...
 /*
      FILE *fp_pframeskip;
      char filename2[50] = "pframe_skip.txt";
      //sprintf(filename2, "%dpframe.txt", pframeNum);
      if (fp_pframeskip = fopen(filename2, "a+"))
      {
        //int count;
        //for (count = 0; count < 64; count++)
        fprintf(fp_pframeskip, "%d ", MODE_NOT_CODED_count/pframeNum);
        //fprintf(fp, "\n");
        fclose(fp_pframeskip);
      }
  */  
